...
%
% VERSION:
%  $Id: SyntheticDataEM.m 2008-03-12 12:54 furche $
%
% AUTHOR:
%  J. Furche 
%
...
%
% DESCRIPTION:
% syntheticDataEM.m uses mainly the functions synthwaveformsgleicheanzahl.m
...
% are obtained from 5 different experiments that have been sorted by the
% automatical T-dist-EM algorithm (the experiments have to be in the same 
...
%
% OUTPUTS:
% Since SyntheticDataEM is computed as a script instead of a function,
...
% overwriting the spike data from an existing NEV-file. The template units
% are obtained from 5 different experiments that have been sorted by the
% automatical T-dist-EM algorithm (the decision which unit should be 
% declared as spike or noise was done manually). For each of the 100
...
% out of the preselected ones in channelauswahlEM.m. After this waveforms 
% and timestamps are generated by synthwaveformsgleicheanzahl.m
...
% NEV-file, further the routines channelauswahlEM.m,
...
for i=1:100; %each electrode
   ...
   if anzahlspikes<4;
       for j=1:anzahlspikes %for each spikeunit:
        [plxnummer,plxdatei,channel,unita]=channelauswahlEM(1); %randomly choosing a channel out of the preselection
        ...
        units=repmat(j,number,1); 
            ...
            if rand<0.15 %15% probability for undershoot. Following operations are analogeous to the spike generation
                [plxnummer,plxdatei,channel,unita]=channelauswahlEM(3);
   ...
   if rauschen==1;
      [plxnummer,plxdatei,channel,unita]=channelauswahlEM(2); 
