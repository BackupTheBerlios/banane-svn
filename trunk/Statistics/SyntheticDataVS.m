...
%
% VERSION:
%  $Id: SyntheticDataVS.m 2008-03-12 12:54 furche $
%
% AUTHOR:
%  J. Furche 
%
...
% automatical Valley seeking algorithm. (the experiments have to be in the 
% same folder as the routines.)
%
%
% CATEGORY:
%  Statistics
...
%
% OUTPUTS:
% Since SyntheticDataVS is computed as a script instead of a function,
...
%
% PROCEDURE:
% syntheticDataVS.m uses mainly the functions synthwaveformsgleicheanzahl.m
% and interspikefktplx2.m to generate a complete spike data file by
% overwriting the spike data from an existing NEV-file. The template units
% are obtained from 5 different experiments that have been sorted by the
% automatical Valley seeking algorithm (the decision which unit should be 
...
% out of the preselected ones in channelauswahlVS.m. After this waveforms and
...
% NEV-file, further the routines channelauswahlVS.m,
...
for i=1:100; %each electrode
   ...
   if anzahlspikes<4;
       for j=1:anzahlspikes %for each spikeunit:
        [plxnummer,plxdatei,channel,unita]=channelauswahlVS(1); %randomly choosing a channel out of the preselection
        ...
        units=repmat(j,number,1); 
            ...
            if rand<0.15 %15% probability for undershoot. Following operations are analogeous to the spike generation
                [plxnummer,plxdatei,channel,unita]=channelauswahlVS(3);
