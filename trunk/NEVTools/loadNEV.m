...
%
% AIM:
%  bla3, Read all important information from a NEV file.
%
% DESCRIPTION:
...
function nevVariable = loadNEV(filename, varargin);
    ...
    % Initialization
    nevVariable.GeneralInfo = struct('timestamps', [], ...
                                     'packetNumbers', [], ...
                                     'packetOrder', [], ...
                                     'unitOrder', [], ...
                                     'NumberSpikes', [], ...
    ...
    % Load in waveform and stimulus information if requested (first
    % version - everything is read in and then sorted) 
    % Only used for smaller files

    % Include this and the elseif below to have it chose between two
    ...
    if wav == 1,
      ...
      % Load in waveform and stimulus information if requested (second
      % version - only relevant channels/ units are loaded in) 
    ...
    disp(' ');
    disp(char(['Filename:    ' ...
               nevVariable.FileInfo.source], ...
              ['Packet count:    ' ...
               num2str(nevVariable.FileInfo.packetCount)], ...
              ['Number of active channels:    ' ...
               num2str(length(find(nevVariable.GeneralInfo.ActiveChannels)))]));